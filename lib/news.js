...
function parseStockWatchlist(value) {
  if (!value) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const items = parsed
        .map((item) => ({
          ...
          label: String(item.label || item.symbol || '').trim(),
    ...
    // Fall back to comma-separated symbols below.
  }
...
async function getStockItems() {
  ...
  const symbols = parseStockWatchlist(process.env.STOCK_WATCHLIST);
